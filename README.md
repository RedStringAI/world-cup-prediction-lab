...
World Cup Prediction Lab 是一个可本地运行的足球模型工作台。它把 FLUX-10 工作流中适合公开的部分整理成可复用产品：十维量化、Elo 差、平局分档校准、双泊松比分矩阵、场景滑杆、JSON 导入导出、回测指标和可选 AI 解读。

它不是静态展示页。打开后可以直接编辑球队参数、调整模型与共识融合比例、导入自己的赛程 JSON、运行推演、查看比分路径，并用真实赛果计算 Brier / RPS / Top3 覆盖。
...
- **可用前端工作台**：比赛队列、参数编辑、雷达图、比分热力图、卡片预览、JSON 导入导出。
